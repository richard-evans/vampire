...
namespace vdc{
   ...
   void rgb( const double& sx, const double& sy, const double& sz, double& red, double& green, double& blue){
      ...
      xy_angle = vdc::scale(0.0, 255.0, 0.0, 2.0*pi, xy_angle);

      // find rgb values
      vdc::interpolate( xy_angle, red, green, blue );
      ...
      // convert to HSL
      vdc::rgb2hsi(red, green, blue, hue, light, saturation);
      ...
      vdc::hsi2rgb(red, green, blue, hue, light, saturation);

      return;
   }


   // function to scale a data set from range [start_min - start_max]
   ...
   // function to convert RGB colour to HSL
   ...
   // function to convert HSL colour to RGB
