...
{\zicf create:full}\phantomsection\addcontentsline{toc}{subsection}{create:full}
Uses the entire generated system without any truncation or consideration of the
\textit{create:particle-size} parameter. \textit{create:full} should be used when
importing a complete system, such as a complete nanoparticle and where a further
definition of the system shape is not required. This is the default if no system
truncation is defined.

{\zicf create:cube}\phantomsection\addcontentsline{toc}{subsection}{create:cube}
Cuts a cuboid particle of size \textit{$l_x$ = $l_y$ = $l_z$ = create:particle-size}
from the defined crystal lattice.

{\zicf create:cylinder}\phantomsection\addcontentsline{toc}{subsection}{create:cylinder}
Cuts a cylindrical particle of diameter \textit{create:particle-size} from the
defined crystal lattice. The height of the cylinder extends to the whole extent
of the system size \textit{create:system-size-z} in the $z$-direction.

{\zicf create:ellipsoid}\phantomsection\addcontentsline{toc}{subsection}{create:ellipsoid}
Cuts an ellipsoid particle of diameter \textit{create:particle-size} with fractional
diameters of \textit{dimensions:particle-shape- factor-x},
\textit{dimensions:particle-shape-factor-y},
\textit{dimensions:particle-shape-factor-z} from the defined crystal lattice.

{\zicf create:sphere}\phantomsection\addcontentsline{toc}{subsection}{create:sphere}
Cuts a spherical particle of diameter \textit{create:particle-size} from the
defined crystal lattice.

{\zicf create:truncated-octahedron}\phantomsection\addcontentsline{toc}{subsection}{create:truncated-octahedron}
Cuts a truncated octahedron particle of diameter \textit{create:particle-size}
from the defined crystal lattice.
...
{\zicf create:granular-film}\phantomsection\addcontentsline{toc}{subsection}{create:granular-film} Generates a two-dimensional granular thin-film structure by partitioning the film footprint into convex grain cells and filling each with atoms from the defined crystal lattice. Grain cells are built as a power (Laguerre) tessellation: every grain seed point carries a weight related to its target size, and the boundary between two neighbouring cells is the radical axis of the two weighted seeds rather than the plain perpendicular bisector of an ordinary Voronoi diagram (the two constructions coincide when every seed has the same weight). This is what allows a requested grain-size distribution to be realised as genuine grain areas rather than only biasing the local density of seed points, and it is controlled by \textit{create:grain-tessellation}. Before the tessellation is built, grain seed positions are found by settling a random packing of discs sized to the requested grain diameters until they come to rest close to their own maximum packing density, rather than being scattered independently of their eventual size; this gives the tessellation a realistic starting point close to the tightly packed appearance of real granular films, so only small corrections are needed to match the requested sizes exactly (see \textit{create:grain-regularity} for an optional further polishing step). This disc-packing relaxation always settles under periodic (minimum-image) boundary conditions internally, regardless of \textit{create:grain-periodic-boundaries}: packing discs against the straight, non-periodic edges of a finite box reliably crystallises a close-to-monodisperse packing into a hexagonal lattice pinned to the box axes, an unphysical wall-ordering artefact unrelated to the requested film shape. Whether the \emph{final} tessellated grain shapes wrap across the film edges or are clipped to a genuinely finite box is controlled separately by \textit{create:grain-periodic-boundaries}, applied only after seed positions are already settled. The mean grain diameter is set by \textit{create:grain-size} (falling back to \textit{dimensions:particle-size} if unset) and the gap left between neighbouring grains by \textit{create:grain-spacing} (falling back to \textit{dimensions:particle-spacing} if unset); this gap is a true constant-width offset of every grain boundary, so it does not scale with grain size. \textit{create:voronoi-film} and \textit{create:grains} are accepted as equivalent, older names for this keyword. Further keywords control the size distribution (\textit{create:grain-size-distribution} and its parameters), corner rounding (\textit{create:grain-rounding}), faceted Wulff-construction grain shapes (\textit{create:grain-facet-symmetry} and related keywords), and periodic wrapping of the grain structure across the film edges (\textit{create:grain-periodic-boundaries}).

{\zicf create:grain-tessellation = voronoi / laguerre [default laguerre]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-tessellation} Selects the geometric engine used to build grain cells for \textit{create:granular-film}. \textbf{laguerre} (the default) is a power diagram: every grain seed carries a weight, fitted iteratively so that each cell's realised area matches its requested target (see \textit{create:grain-size-distribution}). \textbf{voronoi} uses the same half-plane-clipping construction with every seed given equal weight, producing an ordinary Voronoi tessellation whose grain sizes are set only by the local density of seed points rather than by any size request; it is useful mainly as a comparison case, or where a plain Voronoi structure is specifically wanted. For a periodic film (\textit{create:grain-periodic-boundaries}) both engines wrap every cell seamlessly across the film edges; for a non-periodic film (the default), a cell that would otherwise be clipped there is instead removed entirely, leaving a void margin at the film edges rather than a truncated, undersized grain (see \textit{create:grain-periodic-boundaries}).

{\zicf create:grain-size = float [default: falls back to dimensions:particle-size]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-size} Sets the mean grain diameter for \textit{create:granular-film}. For the \textbf{lognormal} and \textbf{bimodal} cases of \textit{create:grain-size-distribution} this is the arithmetic mean of the sampled diameters, not their median. If not set explicitly, \textit{dimensions:particle-size} is used instead, so simple granular-film inputs written without this keyword continue to work unchanged.

{\zicf create:grain-size-distribution = lognormal / normal / bimodal / delta / file [default normal]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-size-distribution} Selects how individual target grain diameters are sampled before the tessellation weights are fitted to realise them. \textbf{normal} (the default) samples from a normal distribution with mean \textit{create:grain-size} and standard deviation \textit{create:grain-size-sd}, giving a naturally varied-looking film without needing either keyword to be set explicitly. \textbf{lognormal} samples from a lognormal distribution with the same mean/sd keywords. \textbf{delta} gives every grain exactly the same diameter \textit{create:grain-size}, i.e.\ a monodisperse film; note that \textit{create:grain-size-sd} and the \textit{create:grain-size-second-*} keywords have no effect under \textbf{delta} (a warning is written to the log if any of them were set while \textbf{delta} is in effect). \textbf{bimodal} mixes two lognormal populations: a majority population as for \textbf{lognormal}, and a minority population with mean diameter \textit{create:grain-size-second}, standard deviation \textit{create:grain-size-second-sd} and number fraction \textit{create:grain-size-second-fraction}. \textbf{file} reads an explicit list of target diameters from \textit{create:grain-size-distribution-file} instead of sampling one.

{\zicf create:grain-size-sd = float [0.0-1.0, default 0.3]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-size-sd} Sets the log-standard deviation of the grain-size distribution for the \textbf{lognormal} and \textbf{normal} cases of \textit{create:grain-size-distribution} (and the majority population of \textbf{bimodal}), as a fraction of the mean diameter. Larger values give a wider spread of realised grain sizes. Has no effect under \textbf{delta}.

{\zicf create:grain-size-second = float [default 2 nm]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-size-second} Sets the mean diameter of the minority (second) grain population when \textit{create:grain-size-distribution} = \textbf{bimodal}. Must be smaller than \textit{create:grain-size}, which continues to set the majority population's mean diameter.

{\zicf create:grain-size-second-sd = float [0.0-1.0, default 0.15]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-size-second-sd} Sets the log-standard deviation of the minority (second) grain population when \textit{create:grain-size-distribution} = \textbf{bimodal}, in the same sense as \textit{create:grain-size-sd}.

{\zicf create:grain-size-second-fraction = float [0.0-1.0, default 0.3]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-size-second-fraction} Sets the target fraction, \textbf{by number} of grains, belonging to the minority population when \textit{create:grain-size-distribution} = \textbf{bimodal}. This is a number fraction, not an area fraction: because grain area scales with diameter squared, a small-diameter population can make up a substantial fraction of the grain count while covering only a small fraction of the film area, e.g.\ a 0.3 number fraction of 2~nm grains mixed with 10~nm grains covers only around 1--2\% of the film area. Both the realised number fraction and the corresponding area fraction are reported to the log file when the system is generated, along with a warning if weight fitting could not bring every seed's realised size within tolerance of its target.

{\zicf create:grain-size-distribution-file = filename}\phantomsection\addcontentsline{toc}{subsection}{create:grain-size-distribution-file} Used with \textit{create:grain-size-distribution} = \textbf{file}. Names a plain-text file containing one target grain diameter (in \AA) per line, or otherwise whitespace-separated; every positive value listed is used directly as the target diameter of one grain, so the file sets both the number of grains and their individual sizes rather than being resampled from.

{\zicf create:grain-regularity = float [0.0-1.0, default 0.0]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-regularity} Applies an optional polishing pass to the settled grain seed positions described under \textit{create:granular-film}, nudging each seed a further step towards the centre of its own cell, repeated a number of times set by this parameter. 0.0 (the default) leaves the seed positions exactly as found by the packing step; larger values give progressively more regular, more hexagonal-looking grain shapes. This affects grain shape and packing regularity only, not the realised grain-size distribution, and is applied before weight fitting adjusts cell sizes to match the requested size distribution.

{\zicf create:grain-structure-random-seed = int [default 1951218893]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-structure-random-seed} Sets the random seed used for grain seed placement, size sampling and facet orientation in \textit{create:granular-film}. A different seed produces a different, but statistically equivalent, random grain structure. Not to be confused with \textit{create:grain-random-seed}, which seeds random alloying \emph{within} an already-generated grain structure and has no effect on the grain structure itself.

{\zicf create:grain-spacing = float [default: falls back to dimensions:particle-spacing]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-spacing} Sets the gap left between neighbouring grains in \textit{create:granular-film}, applied as a constant-width inward offset of every grain boundary. Because the offset is a fixed distance rather than a size-dependent scaling, the same \textit{create:grain-spacing} value leaves the same absolute gap between any two neighbouring grains regardless of their individual sizes. If not set explicitly, \textit{dimensions:particle-spacing} is used instead.

{\zicf create:grain-spacing-sd = float [0.0-1.0, default 0.0]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-spacing-sd} Sets the fractional standard deviation of a random, per-grain jitter applied to \textit{create:grain-spacing} before the boundary gap is opened up, so that the realised gap between any two neighbouring grains becomes the sum of their own two independently-jittered half-gaps rather than an exact constant everywhere. This gives the boundary/segregant network a naturally varying width - typically widest where several large-jitter grains meet at a triple junction and narrowest elsewhere - closer to the non-uniform grain boundaries seen in real sputtered granular media. 0.0 (the default) reproduces the exact constant-width gap of plain \textit{create:grain-spacing}.

{\zicf create:grain-rounding = float [0.0-1.0, default 0.0]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-rounding} Rounds the corners of every grain by clipping its polygon boundary at a common radius from its centre, with the radius chosen so that the retained area is exactly this fraction of the unrounded grain's area. A value of 0.0 (the default) leaves grains unrounded; values close to 1.0 remove only the sharpest corners, while small values give progressively more circular grains. This replaces \textit{create:voronoi-rounded-grains} and \textit{create:voronoi-rounded-grains-area}, which remain accepted as deprecated aliases for this single keyword.

{\zicf create:grain-boundary-roughness = float [0.0-0.6, default 0.0]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-boundary-roughness} Makes the rounded grain outline produced by \textit{create:grain-rounding} irregular rather than uniformly circular, by perturbing the rounding radius with a random combination of low-order angular harmonics, independently for every grain. 0.0 (the default) leaves grain rounding exactly circular; larger values give each grain its own, more irregular, organic ``blob'' outline, closer to the appearance of real granular thin films than a uniformly rounded polygon. Has no effect unless \textit{create:grain-rounding} is also non-zero. The number of harmonics is set by \textit{create:grain-boundary-roughness-modes}.

{\zicf create:grain-boundary-roughness-modes = int [1-20, default 4]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-boundary-roughness-modes} Sets the number of random angular harmonics combined to perturb the grain outline when \textit{create:grain-boundary-roughness} is non-zero. A small number of modes (the default) gives each grain a handful of broad, organic lobes; larger values give higher-frequency, more wobbly outlines.

{\zicf create:grain-facet-symmetry = int [0, 4, 6, 8; default 0]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-facet-symmetry} Enables faceted, Wulff-construction grain shapes and sets the number of facets. Each grain's tessellated cell is clipped by this many planes at equally spaced angles around a per-grain orientation (\textit{create:grain-facet-orientation}), replacing its smooth Laguerre cell boundary with straight facets; this produces the truncated triangular and hexagonal habits seen experimentally in materials such as FePt and CoCrPt granular media. 0 (the default) disables faceting entirely. The facet distances are set so that the corresponding \emph{unclipped} faceted shape would have the same area as the original cell; because a real, irregular cell generally cannot fully contain that shape, faceting an irregular cell typically also trims some area from the grain rather than being a pure reshape (see \textit{create:grain-facet-strength}).

{\zicf create:grain-facet-strength = float [0.0-1.0, default 1.0]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-facet-strength} Sets the strength of the faceting applied when \textit{create:grain-facet-symmetry} is non-zero, blending linearly between the unfaceted tessellated cell (0.0) and the full faceted habit (1.0). Only reached once faceting is enabled, so setting \textit{create:grain-facet-symmetry} alone turns on full-strength faceting immediately.

{\zicf create:grain-facet-anisotropy = float [0.0-10.0, default 1.0]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-facet-anisotropy} Sets the ratio of facet energies between alternating facet families in the Wulff construction used by \textit{create:grain-facet-symmetry}. A value of 1.0 (the default) gives isotropic facet energies and hence a regular polygon habit; values away from 1.0 make alternating facets longer or shorter, producing the truncated, elongated habits seen in real faceted grains and nanoparticles.

{\zicf create:grain-facet-orientation = random / fixed / textured [default random]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-facet-orientation} Sets how the facet orientation angle is chosen for each grain when \textit{create:grain-facet-symmetry} is non-zero. \textbf{random} (the default) gives every grain an independent, uniformly distributed orientation. \textbf{fixed} gives every grain the same orientation, \textit{create:grain-facet-orientation-angle}. \textbf{textured} gives every grain an orientation drawn from a Gaussian distribution centred on \textit{create:grain-facet-orientation-angle} with standard deviation \textit{create:grain-facet-orientation-spread}, useful for modelling a partially textured polycrystalline film. Each grain's orientation angle is stored internally regardless of mode, providing a hook for coupling grain shape to a magnetocrystalline easy axis in future, although nothing currently reads it back out.

{\zicf create:grain-facet-orientation-angle = float [0.0-360.0 degrees, default 0.0]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-facet-orientation-angle} Sets the facet orientation angle used by the \textbf{fixed} and \textbf{textured} modes of \textit{create:grain-facet-orientation}.

{\zicf create:grain-facet-orientation-spread = float [0.0-180.0 degrees, default 15.0]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-facet-orientation-spread} Sets the standard deviation of the Gaussian orientation spread used by the \textbf{textured} mode of \textit{create:grain-facet-orientation}.

{\zicf create:grain-periodic-boundaries flag [default false]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-periodic-boundaries} Wraps the grain tessellation and atom assignment for \textit{create:granular-film} periodically in $x$ and $y$, so that a grain cut by one edge of the film continues seamlessly from the opposite edge instead of losing its territory there. This concerns the grain structure only, and is independent of any exchange or dipole boundary treatment set separately by \textit{create:periodic-boundaries-x}/\textit{-y}/\textit{-z}. The iterative weight fitting used by the default \textbf{laguerre} tessellation engine (\textit{create:grain-tessellation}) matches this setting, so realised grain sizes are fitted against the same wrapped or (see below) surviving cells the final film actually uses either way. Grain seed \emph{placement} itself (the disc-packing relaxation described under \textit{create:granular-film}) always settles under periodic conditions regardless of this flag, so turning this off does not reintroduce the wall-induced ordering that periodic relaxation avoids - it only changes how grains at the film edge are finally treated: wrapped seamlessly (this flag on) versus removed entirely, leaving a void margin (this flag off, the default - see \textit{create:grain-tessellation}).

{\zicf create:grain-statistics-output flag [default false]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-statistics-output} Writes one line per grain to a file named \textit{grain\_statistics.txt} in the working directory, giving its id, centre coordinates, area, equivalent diameter, target diameter, vertex count, an approximate neighbour count, and, for \textbf{bimodal} size distributions, which population it belongs to. Measurements are taken immediately after tessellation and weight fitting, before any spacing offset, rounding or faceting is applied, so they reflect how well the requested size distribution was realised rather than the final, generally smaller, grain shapes. A corresponding summary (realised mean and standard deviation per population, realised number and area fraction for bimodal distributions, and a warning if weight fitting did not fully converge) is written to the log file regardless of this setting.

{\zicf create:voronoi-size-variance = float [0.0-1.0]}\phantomsection\addcontentsline{toc}{subsection}{create:voronoi-size-variance} Deprecated; also accepted as \textit{create:grain-size-variance}. Sets \textit{create:grain-size-sd}, but is not a full replacement for the old behaviour: the seed-placement jitter this keyword used to control is now the separate \textit{create:grain-regularity} keyword, which this alias does not set. A warning identifying the exact substitution made is printed to the log file.

{\zicf create:voronoi-row-offset flag [default false]}\phantomsection\addcontentsline{toc}{subsection}{create:voronoi-row-offset} Deprecated and has no effect. Grain seeds are no longer placed on a hexagonal lattice, so there is no row left to offset; a warning is printed and the keyword is ignored.

{\zicf create:voronoi-random-seed = int}\phantomsection\addcontentsline{toc}{subsection}{create:voronoi-random-seed} Deprecated; use \textit{create:grain-structure-random-seed} instead, which it sets directly.

{\zicf create:voronoi-rounded-grains flag [default false]}\phantomsection\addcontentsline{toc}{subsection}{create:voronoi-rounded-grains} Deprecated; also accepted as \textit{create:rounded-grains}. Sets \textit{create:grain-rounding} = 0.9.

{\zicf create:voronoi-rounded-grains-area = float [0.0-1.0, default 0.9]}\phantomsection\addcontentsline{toc}{subsection}{create:voronoi-rounded-grains-area} Deprecated; also accepted as \textit{create:rounded-grains-area}. Sets \textit{create:grain-rounding} directly to the given value.

{\zicf create:voronoi-include-boundary-grains flag}\phantomsection\addcontentsline{toc}{subsection}{create:voronoi-include-boundary-grains} Deprecated and has no effect. Grain cells touching the boundary of a non-periodic film are always removed (leaving a void margin) rather than clipped and kept, so there is no separate boundary-grain policy left to control; a warning is printed and the keyword is ignored.

{\zicf create:voronoi-bimodal-grains flag}\phantomsection\addcontentsline{toc}{subsection}{create:voronoi-bimodal-grains} Deprecated; sets \textit{create:grain-size-distribution} = \textbf{bimodal}. The companion keywords below set the corresponding \textit{create:grain-size-second-*} values.

{\zicf create:voronoi-small-grain-diameter = float [default 2 nm]}\phantomsection\addcontentsline{toc}{subsection}{create:voronoi-small-grain-diameter} Deprecated; use \textit{create:grain-size-second} instead, which it sets directly.

{\zicf create:voronoi-small-grain-fraction = float [0.0-1.0, default 0.3]}\phantomsection\addcontentsline{toc}{subsection}{create:voronoi-small-grain-fraction} Deprecated; use \textit{create:grain-size-second-fraction} instead, which it sets directly.

{\zicf create:voronoi-small-grain-size-variance = float [0.0-1.0]}\phantomsection\addcontentsline{toc}{subsection}{create:voronoi-small-grain-size-variance} Deprecated; use \textit{create:grain-size-second-sd} instead, which it sets directly.

{\zicf create:voronoi-elliptical-rounding = float [0.0-1.0, default 0.0]}\phantomsection\addcontentsline{toc}{subsection}{create:voronoi-elliptical-rounding}
Controls the degree of elliptical rounding applied to voronoi grains in the direction normal
to the film. In the standard voronoi construction the grain boundaries are vertical, so that
each grain has the same cross-section at every height. This parameter progressively tapers the
grain cross-section away from a chosen height, generating grains which are ellipsoidal in three
dimensions while preserving the granular structure in the plane. A value of 0.0 gives the
standard voronoi structure with vertical grain boundaries, while a value of 1.0 gives fully
ellipsoidal grains which taper to a point at the top and bottom surfaces of the film.
Intermediate values interpolate smoothly between these limits, generating the wider grain
boundaries towards the film surfaces which are typical of sputtered granular media. The
in-plane grain shape at the widest cross-section is always the full voronoi cell, and so
increasing the rounding reduces the total magnetic volume of the system. The height of the
widest cross-section is set by \textit{create:voronoi-elliptical-rounding-height}. This
parameter may be combined with \textit{create:voronoi-rounded-grains}, which rounds the grain
shape within the plane.

{\zicf create:voronoi-elliptical-rounding-height = float [0.0-1.0, default 0.5]}\phantomsection\addcontentsline{toc}{subsection}{create:voronoi-elliptical-rounding-height}
Defines the height of the widest grain cross-section when using
\textit{create:voronoi-elliptical-rounding}, expressed as a fraction of the total system height
\textit{dimensions:system-size-z}. The default value of 0.5 places the widest cross-section at
the centre of the system, generating grains which are symmetric about the mid-plane. Smaller
values move the widest cross-section towards the substrate and larger values towards the free
surface, in both cases generating asymmetric grains which still taper to the top and bottom
surfaces of the film.

{\zicf create:grain-magnetisation-direction = material / alternating [default material]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-magnetisation-direction} Controls how the initial spin direction set by \textit{material[\#]:initial-spin-direction} (see the Material File Command Reference) is applied across the grains of a granular system (e.g. one generated with \textit{create:granular-film}). Grains are numbered sequentially from 0 in the order they are generated. With the default value \textbf{material}, every atom is initialised exactly as specified by its material's texture, with no further modification. With \textbf{alternating}, the initial spin direction is reversed (multiplied by $-1$) for every atom belonging to an odd-numbered grain (grain 1, 3, 5, \ldots), while even-numbered grains (0, 2, 4, \ldots) are left unchanged. This gives neighbouring grains opposite initial magnetisation directions, which is useful for example to initialise antiferromagnetically-coupled grain assemblies or to study domain-wall pinning at grain boundaries. The reversal is applied on top of whichever texture is selected for the material (uniform vector, domain wall, skyrmion, etc.), so it simply flips the sign of the resulting spin direction at each atomic site. Note that with \textit{create:grain-size-distribution} = \textbf{bimodal}, grains are numbered in the (randomised) order their seeds are placed rather than in any spatial or size-related order, so \textbf{alternating} will not correlate with grain size or position.

{\zicf create:grain-shape-output flag [default false]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-shape-output} Writes the vertices of every generated grain (e.g. from \textit{create:granular-film}) to a file named \textit{grain\_shapes.txt} in the working directory, one grain per line, giving the number of vertices followed by their $x$,$y$ coordinates. This is disabled by default; set this flag to enable it when the grain geometry is needed for analysis or plotting.
...
{\zicf create:grain-random-seed integer [default 1527349271]}\phantomsection\addcontentsline{toc}{subsection}{create:grain-random-seed} Sets the random seed for the pseudo-random number generator used for granular alloying, i.e.\ randomly assigning materials to grains (see \textit{material:host-alloy} = \textbf{granular} in the Material File Command Reference). This does \emph{not} affect the grain structure itself, which is a separate random process seeded by \textit{create:grain-structure-random-seed}.
...
{\zicf dimensions:system-size = float [0.0 - 10 $\mu$ m, default 7.7 nm}\phantomsection\addcontentsline{toc}{subsection}{dimensions:system-size}
Defines the size of the symmetric bulk crystal in $x$, $y$ and $z$. Note that
large system sizes will generate large numbers of atoms and will require
supercomputing resources.

{\zicf dimensions:system-size-x = float [0.0 - 10 $\mu$ m, default 7.7 nm}\phantomsection\addcontentsline{toc}{subsection}{dimensions:system-size-x}
Defines the total size if the system along the $x$-axis. Note that
large system sizes will generate large numbers of atoms and will require
supercomputing resources.

{\zicf dimensions:system-size-y = float [0.0 - 10 $\mu$ m, default 7.7 nm}\phantomsection\addcontentsline{toc}{subsection}{dimensions:system-size-y}
Defines the total size if the system along the $y$-axis. Note that
large system sizes will generate large numbers of atoms and will require
supercomputing resources.

{\zicf dimensions:system-size-z = float [0.0 - 10 $\mu$ m, default 7.7 nm}\phantomsection\addcontentsline{toc}{subsection}{dimensions:system-size-z}
Defines the total size if the system along the $z$-axis. Note that
large system sizes will generate large numbers of atoms and will require
supercomputing resources.
...
{\zicf dimensions:particle-shape-factor-x = float [0.001-1, default 1.0]} \phantomsection\addcontentsline{toc}{subsection}{dimensions:particle-shape-factor-x}
...
{\zicf dimensions:particle-shape-factor-y = float [0.001-1, default 1.0]} \phantomsection\addcontentsline{toc}{subsection}{dimensions:particle-shape-factor-y}
Modifies the default particle shape to create elongated particles. The selected
particle shape is modified by changing the effective particle size in the $y$
direction. This property scales the as a fraction of the particle-size along the
$y$-direction.

{\zicf dimensions:particle-shape-factor-z = float [0.001-1, default 1.0]}\phantomsection\addcontentsline{toc}{subsection}{dimensions:particle-shape-factor-z}
Modifies the default particle shape to create elongated particles. The selected
particle shape is modified by changing the effective particle size in the $z$
direction. This property scales the as a fraction of the particle-size along the
$z$-direction.

{\zicf dimensions:particle-array-offset-x [0-10$^4$ \AA]}\phantomsection\addcontentsline{toc}{subsection}{dimensions:particle-array-offset-x}
Translates the 2-D particle array the chosen distance along the x-direction.

{\zicf dimensions:particle-array-offset-y}\phantomsection\addcontentsline{toc}{subsection}{dimensions:particle-array-offset-y}
Translates the 2-D particle array the chosen distance along the y-direction.

{\zicf dimensions:double macro-cell-size}\phantomsection\addcontentsline{toc}{subsection}{dimensions:macro-cell-size}
...
should always be less than the system size, as highly asymmetric cells will lead
to significant errors in the demagnetisation field calculation.

\section*{Exchange calculation}\phantomsection\addcontentsline{toc}{section}{Exchange calculation}
The following commands control the calculation of built-in exchange interactions
for the system.

{\zicf exchange:interaction-range}\phantomsection\addcontentsline{toc}{subsection}{exchange:interaction-range double default [1.0]}
Determines the cutoff range exchange interactions for built-in crystal structures
in terms of the nearest neighbour range. Larger ranges will enable more interactions
via an exchange function which can include 2nd-10th nearest neighbour interaction
shells or exponential functions. Note that longer ranged interactions are slower
to calculate. In shell mode the computed interaction shells are printed in the
log file.

{\zicf exchange:function}\phantomsection\addcontentsline{toc}{subsection}{exchange:function string default [nearest-neighbour]}
Determines the type of interaction to be used in the spin Hamiltonian. The
default \textit{nearest-neighbour} option forces nearest neighbour interactions
only. The \textit{shell} option groups neighbours at the same interaction
distance into shells which can then be assigned different exchange constants.
The \textit{exponential} option implements an exponential decay that is useful
for simulating spin glasses and systems such as NdFeB where there are no
well-defined neighbour shells. The \textit{material-exponential} function is
similar to \textit{exponential}, however it allows different exponential
exchange functions to be defined for different inter-material type (for
materials as defined in the unit-cell module) interactions e.g. in NdFeB
Nd-Fe interactions can have a different function defined vs Fe-Fe interactions.

{\zicf exchange:decay-multiplier}\phantomsection\addcontentsline{toc}{subsection}{exchange:decay-multiplier double default [1.0]}
Determines the value of $A$ to be used in $A\exp{-r/B}+C$ for
\textit{exchange:function} = \textit{exponential}.

{\zicf exchange:decay-length}\phantomsection\addcontentsline{toc}{subsection}{exchange:decay-length double default [0.4]}
Determines the value of $B$ to be used in $A\exp{-r/B}+C$ for
\textit{exchange:function} = \textit{exponential}.

{\zicf exchange:decay-shift}\phantomsection\addcontentsline{toc}{subsection}{exchange:decay-shift double default [0.0]}
Determines the value of $C$ to be used in $A\exp{-r/B}+C$ for
\textit{exchange:function} = \textit{exponential}.

{\zicf exchange:ucc-exchange-parameters[i][j]}\phantomsection\addcontentsline{toc}{subsection}{exchange:ucc-exchange-parameters[i][j] default [1.0, 0.4, 0.0]}
This is used in conjunction with \textit{exchange:function} =
\textit{material-exchange}. i and j represent the unit cell category (material
as per unit cell module) of the interacting atoms that the user wishes to set
the exponential exchange function for. This variable is set to the three comma
separated values: $A$, \textit{exchange:decay-multiplier}; $B$,
\textit{exchange:decay-length}; $C$, \textit{exchange:decay-shift} in this order.

{\zicf exchange:dmi-cutoff-range}\phantomsection\addcontentsline{toc}{subsection}{exchange:dmi-cutoff-range double default [1.0]}
...
\phantomsection\addcontentsline{toc}{section}{Anisotropy calculation}
The following commands control the calculation of the magnetic anisotropy energy
for the system.

{\zicf anisotropy:surface-anisotropy-threshold = integer [default native]}\phantomsection\addcontentsline{toc}
{subsection}{anisotropy:surface-anisotropy-threshold} Determines minimal number
of neighbours to classify as surface atom. The default value is the number of
neighbours specified by the crystal or unit cell file. You can set this as a
lower threshold.

{\zicf anisotropy:surface-anisotropy-nearest-neighbour-range = float [default $\infty$]}\phantomsection\addcontentsline{toc}
{subsection}{anisotropy:surface-anisotropy-nearest-neighbour-range} Sets the
interaction range for the nearest neighbour list used for the surface anisotropy
calculation.

{\zicf anisotropy:enable-bulk-neel-anisotropy = bool [default false]}\phantomsection\addcontentsline{toc}
{subsection}{anisotropy:enable-bulk-neel-anisotropy} Enables calculation of the
N\'eel pair anisotropy in the bulk, irrespective of the number of neighbours,
enabling the effect of localised spin-orbit interactions. Internally this sets a
large threshold, and so specifying anisotropy:surface-anisotropy-threshold will
override this flag.

{\zicf anisotropy:neel-anisotropy-exponential-range = float [default $2.5$]}\phantomsection\addcontentsline{toc}
{subsection}{anisotropy:neel-anisotropy-exponential-range} Enables an
exponentially range dependent N\'eel pair anisotropy so that lattice distortions
and strains change the magnetoelastic compling strength. In the usual form the
method only takes into account the symmetry ($L_{ij}(r) = const$). The value
should be set to the typical lattice parameter otherwise the total anisotropy
will be significantly higher or lower than expected. The functional form of the
range dependence is
...
\noindent where $r_{ij}$ is the pair separation, $r_0$ is the exponential range,
...
the N\'eel anisotropy constant is the same as would be without the
range-dependent form.

{\zicf anisotropy:neel-anisotropy-exponential-factor = float [default $5.52$]}\phantomsection\addcontentsline{toc}
{subsection}{anisotropy:neel-anisotropy-exponential-factor} Enables an
exponentially range dependent N\'eel pair anisotropy so that lattice distortions
and strains change the magnetoelastic compling strength. In the usual form the
method only takes into account the symmetry ($L_{ij}(r) = const$). The prefactor
controls the falloff with increasing range.
...
{\zicf spin-lattice:potential-cutoff-range=float}\phantomsection\addcontentsline{toc}{subsubsection}{spin-lattice:potential-cutoff-range=float}
Distance cutoff for the interatomic potential calculations. Units is Angstrom.  Default value is 7.8 !A;

{\zicf spin-lattice:fields-cutoff-range=float}\phantomsection\addcontentsline{toc}{subsubsection}{spin-lattice:fields-cutoff-range=float}
Distance cutoff for the interatomic potential calculations. Units is Angstrom.  Default value is 3.75 !A;

{\zicf spin-lattice:coupling=string}\phantomsection\addcontentsline{toc}{subsubsection}{spin-lattice:coupling=string}
 Flag that sets up the type of spin-lattice coupling. Possible values are : pseudodipolar, {neel-full};

 {\zicf spin-lattice:potential=string}\phantomsection\addcontentsline{toc}{subsubsection}{spin-lattice:potential=string}
 Flag that sets up the type of potential. Possible values are : harmonic, morse, embedded;


\section*{Phonon Pump Parameters}
The following commands control an external, time-dependent force pump designed to excite specific phonon modes in the lattice. This is used to simulate the effects of THz laser pulses on the spin-lattice system. All parameters are defined using the \textit{phonon:} prefix.

{\zicf phonon:linear-pump = boolean}\phantomsection\addcontentsline{toc}{subsubsection}{phonon:linear-pump = boolean}
Flag to enable the linearly polarized phonon pump. If enabled, a force is applied to the atoms according to the parameters below. Default value is false.

{\zicf phonon:frequency = float ! string}\phantomsection\addcontentsline{toc}{subsubsection}{phonon:frequency = float}
Specifies the frequency of the driving force. The value is converted to base SI units (Hz). It is recommended to specify units. Supported units: THz, GHz. Example: \textit{phonon:frequency = 8.3 ! THz}.

{\zicf phonon:pulse-start-time = float ! string}\phantomsection\addcontentsline{toc}{subsubsection}{phonon:pulse-start-time = float}
Specifies the simulation time at which the force pump turns on. The value is converted to base SI units (s). It is recommended to specify units. Supported units: ps, fs. Example: \textit{phonon:pulse-start-time = 5.0 ! ps}.

{\zicf phonon:pulse-end-time = float ! string}\phantomsection\addcontentsline{toc}{subsubsection}{phonon:pulse-end-time = float}
Specifies the simulation time at which the force pump turns off. The value is converted to base SI units (s). It is recommended to specify units. Supported units: ps, fs. Example: \textit{phonon:pulse-end-time = 80.0 ! ps}.

{\zicf phonon:force-amplitude-x = float}\phantomsection\addcontentsline{toc}{subsubsection}{phonon:force-amplitude-x = float}
The amplitude of the force component applied in the x-direction. For a linear pump, this is the main driving force. For a chiral pump, this could be the amplitude of the cosine component. Units are Newtons (N).

{\zicf phonon:force-amplitude-y = float}\phantomsection\addcontentsline{toc}{subsubsection}{phonon:force-amplitude-y = float}
The amplitude of the force component applied in the y-direction. For a chiral pump, this could be the amplitude of the sine component. Units are Newtons (N).

{\zicf phonon:force-amplitude-z = float}\phantomsection\addcontentsline{toc}{subsubsection}{phonon:force-amplitude-z = float}
The amplitude of the force component applied in the z-direction. Units are Newtons (N).

{\zicf phonon:wave-lambda-x = float}\phantomsection\addcontentsline{toc}{subsubsection}{phonon:wave-lambda-x = float}
The wavelength ($\lambda_x$) of the force wave in the x-direction. Used to calculate the final k-vector. Units are in Angstrom. 

{\zicf phonon:wave-lambda-y = float}\phantomsection\addcontentsline{toc}{subsubsection}{phonon:wave-lambda-y = float}
The wavelength ($\lambda_y$) of the force wave in the y-direction. Units are in Angstrom.

{\zicf phonon:wave-lambda-z = float}\phantomsection\addcontentsline{toc}{subsubsection}{phonon:wave-lambda-z = float}
The wavelength ($\lambda_z$) of the force wave in the z-direction. Units are in Angstrom. 

\{\zicf phonon:wave-direction-x = float\}\phantomsection\textbackslash{}addcontentsline\{toc\}{subsubsection\}{phonon:wave-direction-x = float\} The x-component of the unit vector that defines the direction of propagation for the force wave. 

\{\zicf phonon:wave-direction-y = float\}\phantomsection\textbackslash{}addcontentsline\{toc\}{subsubsection\}{phonon:wave-direction-y = float\} The y-component of the unit vector that defines the direction of propagation. 

\{\zicf phonon:wave-direction-z = float\}\phantomsection\textbackslash{}addcontentsline\{toc\}{subsubsection\}{phonon:wave-direction-z = float\} The z-component of the unit vector that defines the direction of propagation. 
...
default fall and rise times are 0, reproducing a square pulse. An optional delay
before the pulse begins can be set with \textit{sim:electrical-pulse-delay}. The time
...
{\zicf sim:program = laser-electrical-pulse}\phantomsection\addcontentsline{toc}{subsubsection}{laser-electrical-pulse}
Simulates the combined effect of an ultrafast laser pulse and an independent electrical pulse.
The laser pulse heats the electronic and phonon subsystems using the two-temperature model (TTM),
driven by the same parameters as \textit{sim:program = temperature-pulse}
(\textit{sim:laser-pulse-temporal-profile}, \textit{sim:laser-pulse-time}, \textit{sim:laser-pulse-power},
\textit{sim:two-temperature-electron-heat-capacity}, \textit{sim:two-temperature-phonon-heat-capacity},
\textit{sim:two-temperature-electron-phonon-coupling}, \textit{sim:equilibration-temperature}).
The electrical pulse modulates the spin-transfer or spin-orbit torque strength via a Gaussian envelope,
scaled by the STT/SOT field strengths set in the material file.
The electrical Gaussian is centred at $t_\mathrm{delay} + 3\tau_\mathrm{elec}$ from the start of the simulation,
where $t_\mathrm{delay}$ is set by \textit{sim:electrical-pulse-delay} and $\tau_\mathrm{elec}$ by
\textit{sim:electrical-pulse-time}.
The laser Gaussian peaks at $3\tau_\mathrm{laser}$, so setting delay $= 0$ with equal pulse times produces
perfectly overlapping pulses; a positive delay shifts the electrical pulse to arrive after the laser.
The fractional electrical field strength can be written to the output file with
\textit{output:fractional-electric-field-strength}.

{\zicf sim:program = field-pulse}\phantomsection\addcontentsline{toc}{subsubsection}{field-pulse}
...
{\zicf sim:electrical-pulse-delay = float [default $0.0$ s]}\phantomsection\addcontentsline{toc}{subsection}{sim:electrical-pulse-delay}
Defines a time delay before the electrical pulse begins, with default units of seconds and a default of 0.
In \textit{sim:program = electrical-pulse} the trapezoidal pulse is shifted forward in time by this amount,
so the rise phase does not begin until $t = t_\mathrm{delay}$.
In \textit{sim:program = laser-electrical-pulse} it sets the offset from the laser pulse centre to the
electrical pulse centre; note that in this program \textit{sim:electrical-pulse-time} specifies the
Gaussian width $\tau_\mathrm{elec}$ rather than a trapezoidal constant-phase duration.

 {\zicf sim:field-pulse-time = float [default $1.0$ ns]}\phantomsection\addcontentsline{toc}{subsection}{sim:field-pulse-time}
...
{\zicf output:spin-temperature}\phantomsection\addcontentsline{toc}{subsection}{output:spin-temperature} outputs the spin-temperature. Also possible to output the time-averaged values: mean-spin-temperature and material specific values: material-spin-temperature, mean-material-spin-temperature

{\zicf output:lattice-temperature}\phantomsection\addcontentsline{toc}{subsection}{output:lattice-temperature} outputs the lattice-temperature. Also possible to output the time-averaged values: mean-lattice-temperature and material specific values: material-lattice-temperature, mean-material-lattice-temperature
...
{\zicf output:mean-coupling-field-vec}\phantomsection\addcontentsline{toc}{subsection}{output:mean-coupling-field-vec}Outputs the three vector components of the mean pseudodipolar coupling field ($H_c$) averaged over all atoms. The output consists of three columns: Mean\_Hc\_x(T), Mean\_Hc\_y(T), and Mean\_Hc\_z(T). Units are Tesla (T).
