...
%{\zicf material:total-exchange-matrix[j] = float [default 12x5.6 J/link]}\addcontentsline{toc}{subsection}{material:second-uniaxial-anisotropy-constant} defines the total exchange energy between atoms of type index  and  neighbour-index. The  total exchange energy is divided equally between the nearest neighbouring atoms. Unlike the pair wise exchange energy, the total exchange energy also corrects for the spin wave fluctuations arising from the coordination number in the bulk crystal. As with the pairwise exchange energy, the total exchange energy must be defined between all material pairs in the simulation, with positive values representing ferromagnetic coupling, and negative values representing anti ferromagnetic coupling. For a ferromagnet with nearest neighbour exchange, the total exchange energy can be found from the Curie temperature by the meanfield expression:

%where Jij is the exchange energy, Kbps is the Boltzmann constant, and Tc is the Curie temperature. If a custom unit cell is used the exchange values defined here are ignored.\\
...
{\zicf material:uniaxial-anisotropy-constant = float [default 0.0 J/atom]}\addcontentsline{toc}{subsection}{material:uniaxial-anisotropy-constant}
...
{\zicf material:second-uniaxial-anisotropy-constant = float [default 0.0 J/atom]}\addcontentsline{toc}{subsection}{material:second-uniaxial-anisotropy-constant}
...
Where $\sms_i$ is the local spin direction and \ei is the easy axis unit vector. Positive values of \kuuuu give a preferred easy axis orientation, and negative values give a preferred easy plane orientation of the spin.\\
  ...
  %lattice-anisotropy-constant
  %lattice-anisotropy-inflection-temperature
  %lattice-anisotropy-unity-temperature
  %lattice-anisotropy-temperature-width

{\zicf material:cubic-anisotropy-constant = float [default 0.0 J/atom]}\addcontentsline{toc}{subsection}{material:cubic-anisotropy-constant}
...
{\zicf material:uniaxial-anisotropy-direction = float vector [default (001)]}\addcontentsline{toc}{subsection}{material:uniaxial-anisotropy-direction} A unit vector \ei describing the magnetic easy axis direction for uniaxial anisotropy. The vector is entered in comma delimited form - For example:\\
...
%{\zicf material:uniaxial-anisotropy-tensor float tensor [default 0]}\addcontentsline{toc}{subsection}{material:uniaxial-anisotropy-tensor}
...
{\zicf material:surface-anisotropy-constant = float default 0.0 (J/atom)}\addcontentsline{toc}{subsection}{material:surface-anisotropy-constant} Describes the surface anisotropy constant in the N\'eel pair anisotropy model. The anisotropy is given by a summation over nearest neighbour atoms given by
\begin{equation*}
