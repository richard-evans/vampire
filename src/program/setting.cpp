...
//---------------------------------------------------------------------------------------------------------------------
// Setting algorithm for exchange bias in IrMn3 (serial only -- fix me)
//---------------------------------------------------------------------------------------------------------------------
void setting_process(){

	std::cout<< sim::H_applied << "\t" << sim::H_vec[0] << "\t" << sim::H_vec[1] << "\t" << sim::H_vec[2] <<std::endl;

	// Manually set possible orientations of sublattices
	...
	// vector for storing number of atoms in each sublattice
	std::vector<int> No_in_Sublattice(4);

	// check calling of routine if error checking is activated
	if(err::check==true) std::cout << "program::setting has been called" << std::endl;

	// loop over all atoms to calculate number of atoms in afm interface
	...
	int Direction_Closest_to_Field;

	// loop over possible directions to determine closest one to setting field
	for (int i = 0; i < 8; i ++){

		//std::cout<< Configurations[i][0]*sim::H_vec[0] << "\t" << Configurations[i][1]*sim::H_vec[1] << "\t" << Configurations[i][2]*sim::H_vec[2] <<std::endl;
		double result = Configurations[i][0]*sim::H_vec[0] + Configurations[i][1]*sim::H_vec[1] + Configurations[i][2]*sim::H_vec[2];
	...
	// Set sublattice to correct order based on geometry
	int Chosen_array;
	for (int i = 0; i <8; i ++){
	...
	// Set atomic directions according to the sublattice configuration
	for (int i = 0; i <atoms::num_atoms; i++){
