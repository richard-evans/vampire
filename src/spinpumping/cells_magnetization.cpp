...
void calculate_cell_magnetization(const unsigned int num_local_atoms,            // number of local atoms
   ...
   //---------------------------------------------------------------------------
   std::fill(spin_pumping::internal::cell_magnetization.begin(), spin_pumping::internal::cell_magnetization.end(), 0.0);
   ...
   for(unsigned int atom = 0; atom < num_local_atoms; atom++){
      ...
      // get cell id
      const uint64_t cell = spin_pumping::internal::atom_in_cell[atom];

      // get magnetic moment (muB)
      const double mm = atoms_m_spin_array[atom];

      // add magnetization to cell
      spin_pumping::internal::cell_magnetization[3*cell+0] += mm*atoms_x_spin_array[atom];
      spin_pumping::internal::cell_magnetization[3*cell+1] += mm*atoms_y_spin_array[atom];
      spin_pumping::internal::cell_magnetization[3*cell+2] += mm*atoms_z_spin_array[atom];
   ...
   #ifdef MPICF
      // cast to int for MPI
      int bufsize = 3*spin_pumping::internal::total_num_cells;
      MPI_Allreduce(MPI_IN_PLACE, &spin_pumping::internal::cell_magnetization[0], bufsize, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
   #endif
