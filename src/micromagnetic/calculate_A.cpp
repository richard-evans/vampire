...
namespace internal{
      ...
      std::vector< double > calculate_a(int num_atoms,
            ...
            //-------------------------------------------------------------
            // Variables for exchange calculation
            //-------------------------------------------------------------
            int array_index = 0;
            std::vector< std::vector< double> > a2d; //stores the 2D exchange constants.
            a2d.resize(num_cells, std::vector<double>(num_cells,0.0)); // this could possibly be local cells - much better memory scaling?

            std::vector<double> a; //1D vector to store the 2D vector odf the exchange intereactions
            a.resize(num_cells*num_cells,0.0); // this can be determined later to be only as big as necessary
            ...
            // Calculate exchange interactions based on scalar, vector or tensor exchange
            switch(exchange::get_exchange_type()){
               ...
               //   std::cout << cell << '\t' << mat << std::endl;
                  //the neighbours are stored in an array - for each atom the start and end index for the array are found,
                  const int start = atoms::neighbour_list_start_index[atom];
                  const int end   = atoms::neighbour_list_end_index[atom] + 1;
                  //loops over all neighbours
                  for(int nn=start;nn<end;nn++){
                     //calculated the atom id and cell id of the nn atom
                     const int natom = atoms::neighbour_list_array[nn];
                     const int ncell = cell_array[natom];
                     //if interaction is across a cell boundary
                     if(ncell !=cell){
                        // calculate the distance between the two atoms (this doesnt work for periodic boundaries)
