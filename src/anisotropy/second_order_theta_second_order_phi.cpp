   ...
   namespace internal{
      ...
      void second_order_theta_second_order_phi_fields(std::vector<double>& spin_array_x,
         ...
         for(int atom = start_index; atom < end_index; atom++){
            ...
            field_array_x[atom] += 2*k2r2*Sx*fx;
            field_array_y[atom] += 2*k2r2*Sx*fy;
            field_array_z[atom] += 2*k2r2*Sx*fz;

            // sum y-component of field, where y-direction is represented by gx, gy, gz
            field_array_x[atom] -= 2*k2r2*Sy*gx;
            field_array_y[atom] -= 2*k2r2*Sy*gy;
            field_array_z[atom] -= 2*k2r2*Sy*gz;
