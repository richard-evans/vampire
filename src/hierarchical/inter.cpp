...
//     vutil::vtimer_t timer;
   ...
   //timer.start();
    ...
    double tmp_rij_inter_zz = 0.0;
       ...
       if (rij/cells::macro_cell_size > dipole::cutoff){
   ...
   //   vmpi::barrier();
     ...
     // zlog << zTs() << "cell\t" << cell_i <<   "\tPrecalculation of rij matrix for dipole calculation complete. Time taken: " << timer.elapsed_time() << " s"<< std::endl;
         ...
         //--------------------------------------------------------------------------
         else if(rij/cells::macro_cell_size <= dipole::cutoff){
      ...
      //   std::cout<<pi << '\t' << cell_i << '\t' << cells_atom_in_cell_coords_array_x[cell_i][pi] << '\t' << cells_atom_in_cell_coords_array_y[cell_i][pi] << '\t' << cells_atom_in_cell_coords_array_z[cell_i][pi] << std::endl;
   ...
   //      std::cout<<pi << '\t' << qj << '\t' << ex << '\t' << ey << '\t' << ez << std::endl;
         ...
         //std::cout << rij3 << '\t' << ex << '\t' << ey << '\t' << ez << '\t' << cell_i << '\t' << cell_j << '\t' << ha::num_atoms_in_cell[cell_i] << '\t' << ha::num_atoms_in_cell[cell_j] << '\t' << dx << '\t' << dy << '\t' << dz <<tmp_rij_inter_xx << '\t' << tmp_rij_inter_xy << '\t' << tmp_rij_inter_xz << '\t' <<std::endl;
                }
             }
