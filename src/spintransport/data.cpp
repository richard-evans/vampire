...
namespace spin_transport{
   ...
   namespace internal{
      ...
      std::vector <double> cell_sl_resistance;
      std::vector <double> cell_sl_spin_resistance;
      ...
      std::vector <bool>   sl_magnetic;                  // boolean array to determine if cell is magnetic or not, by material type
      std::vector <double> cell_sl_alpha;                // cell magnetization (average of constituent atoms)
      std::vector <double> cell_sl_isaturation;          // inverse magnetic saturation at T=0 in each cell
      std::vector <double> cell_sl_relaxation_torque_rj; // cell specific prefactors for spin-torque relaxation bj
      std::vector <double> cell_sl_precession_torque_pj; // cell specific prefactors for spin-torque precession aj
      std::vector <double> cell_sl_magnetization_x;        // normalised magnetization of each material in each cell
      std::vector <double> cell_sl_magnetization_y;        // normalised magnetization of each material in each cell
      std::vector <double> cell_sl_magnetization_z;        // normalised magnetization of each material in each cell
      std::vector <double> cell_sl_spin_torque_fields_x;   // array of cell spin torque fields x
      std::vector <double> cell_sl_spin_torque_fields_y;   // array of cell spin torque fields y
      std::vector <double> cell_sl_spin_torque_fields_z;   // array of cell spin torque fields z
