...
namespace internal{
   ...
   void read_biquadratic_interactions(unit_cell_t & unit_cell,
      ...
      const int num_exchange_values = exchange::set_biquadratic_exchange_type(exchange_type_string);

      if(num_interactions>=0) unit_cell.biquadratic_interaction.resize(num_interactions);
      else {
      ...
      std::cout << "done!\nProcessing biquadratic data from " << num_interactions << " interactions..." << std::flush;
      zlog << zTs() << "\t" << "Processing unit cell atoms completed" << std::endl;
      zlog << zTs() << "\t" << "Processing data from " << num_interactions << " biquadratic interactions..." << std::endl;
      ...
      for (int i=0; i<num_interactions; i++){
         ...
         unit_cell.biquadratic_interaction[i].dx=dx;
         unit_cell.biquadratic_interaction[i].dy=dy;
         unit_cell.biquadratic_interaction[i].dz=dz;
         ...
         switch(num_exchange_values){
            case 1:
               int_iss >> unit_cell.biquadratic_interaction[i].Jij[0][0];
               // save interactions into diagonal components of the exchange tensor
               unit_cell.biquadratic_interaction[i].Jij[1][1] = unit_cell.biquadratic_interaction[i].Jij[0][0];
               unit_cell.biquadratic_interaction[i].Jij[2][2] = unit_cell.biquadratic_interaction[i].Jij[0][0];
               break;
            case 3:
               int_iss >> unit_cell.biquadratic_interaction[i].Jij[0][0] >> unit_cell.biquadratic_interaction[i].Jij[1][1] >> unit_cell.biquadratic_interaction[i].Jij[2][2];
               break;
            case 9:
               int_iss >> unit_cell.biquadratic_interaction[i].Jij[0][0] >> unit_cell.biquadratic_interaction[i].Jij[0][1] >> unit_cell.biquadratic_interaction[i].Jij[0][2];
               int_iss >> unit_cell.biquadratic_interaction[i].Jij[1][0] >> unit_cell.biquadratic_interaction[i].Jij[1][1] >> unit_cell.biquadratic_interaction[i].Jij[1][2];
               int_iss >> unit_cell.biquadratic_interaction[i].Jij[2][0] >> unit_cell.biquadratic_interaction[i].Jij[2][1] >> unit_cell.biquadratic_interaction[i].Jij[2][2];
               break;
            default:
               terminaltextcolor(RED);
