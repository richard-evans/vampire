...
void build_rutile(unitcell::unit_cell_t& unit_cell){
   ...
   unit_cell.lcsize=2;
   unit_cell.hcsize=2;
   unit_cell.interaction_range=2; //interaction extends longer than standard bcc
   unit_cell.atom.resize(2);
   unit_cell.surface_threshold=14;

   // 0,0,0 and 0.5,0.5,0.5 atoms only magnetic 
   // tetrahedral remainder furthcoming 
   //-----------------------------
   ...
   unit_cell.cutoff_radius =1.0;// sqrt(3.0)/2.0; // normalised to unit cell size
