...
//
//   (c) Pawel Sobieszczyk and Richard F L Evans 2019. All rights reserved.
//
...
//									hex_particle_array
//
//					Function to cut many particles from lattice
//					in a hexagonal configuration
//------------------------------------------------------------------------------
void hex_particle_array(std::vector<cs::catom_t> & catom_array){
   ...
   int num_y_particle = vmath::iceil(cs::system_dimensions[1]/(repeat_size*sqrt(3)/2));
   ...
   for (int x_particle=0;x_particle < num_x_particle;x_particle++){
      for (int y_particle=0;y_particle < num_y_particle;y_particle++){
         ...
         particle_origin[0] = double(x_particle)*repeat_size + fmod(y_particle, 2)*repeat_size/2 + cs::particle_scale*0.5 + cs::particle_array_offset_x;
         particle_origin[1] = double(y_particle)*repeat_size*sqrt(3)/2+cs::particle_scale*0.5 + cs::particle_array_offset_y;
