...
//
//   (c) R F L Evans 2019. All rights reserved.
//
//-----------------------------------------------------------------------------
...
namespace internal{
   ...
   void ellipse(std::vector<double>& particle_origin,std::vector<cs::catom_t> & catom_array, const int grain){
      //--------------------------------------------------------------------------------------------
      //
      ///  Function to cut an ellipsoid particle shape
      ///
      ///  (C) R F L Evans 30/1/2019
      ///
      ...
      ///   r_x^2     r_y^2
      ///
      ///  Evaluation of the above for particle coordinates h,k
      ///  and atomic positions x,y will include all atoms within
      ///  ellipse with vertices r_x and r_y.
      //
      //--------------------------------------------------------------------------------------------
      ...
      for(int atom=0;atom<num_atoms;atom++){
         ...
         if(mp::material[catom_array[atom].material].core_shell_size>0.0){
            ...
            for(std::list<core_radius_t>::iterator it = material_order.begin(); it !=  material_order.end(); it++){
               ...
               if(range_x_sq * inv_rx_sq + range_y_sq * inv_ry_sq <= my_radius_sq){
         ...
         else if(range_x_sq * inv_rx_sq + range_y_sq * inv_ry_sq <= 1.0) {
            catom_array[atom].include = true;
            catom_array[atom].grain = grain;
         }
      }
