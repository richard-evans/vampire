...
//         	const double self_demag = demag::prefactor*eightPI_three_cell_volume;
            ...
            fprintf(stderr,"cell %d magx=%f magy=%f magz=%f rank=%d\n",i,cells::mag_array_x[i],cells::mag_array_y[i],cells::mag_array_z[i],vmpi::my_rank);
            fprintf(stderr,"cell %d fieldx=%f fieldy=%f fieldz=%f rank=%d\n",i,cells::field_array_x[i],cells::field_array_y[i],cells::field_array_z[i],vmpi::my_rank);

         	// Loop over all other cells to calculate contribution to local cell
         	for(int j=0;j<dipole::internal::cells_num_cells;j++){
               fprintf(stderr,"j = %d dipole::internal::num_cells = %d dipole::internal::cells_num_atoms_in_cell %d on my_rank = %d\n",j,dipole::internal::cells_num_cells,dipole::internal::cells_num_atoms_in_cell[j],vmpi::my_rank);
                  ...
                  //#else
            		if( i!=j ){
                  //#endif
                     fprintf(stderr," >> inter >> j = %d, x = %f y = %f z = %f M = %e on rank = %d\n",j,dipole::internal::cells_pos_and_mom_array[5*j+0],dipole::internal::cells_pos_and_mom_array[4*j+1],dipole::internal::cells_pos_and_mom_array[4*j+2],dipole::internal::cells_pos_and_mom_array[4*j+3],vmpi::my_rank);
                     ...
                     fprintf(stderr,"\t%f*%f %f*%f %f*%f\n\t%f*%f %f*%f %f*%f\n\t%f*%f %f*%f %f*%f\n\t on rank = %d\n",mx,internal::rij_inter_xx[lc][j],my,internal::rij_inter_xy[lc][j],mz,internal::rij_inter_xz[lc][j],mx,internal::rij_inter_xy[lc][j],my,internal::rij_inter_yy[lc][j],mz,internal::rij_inter_yz[lc][j],mx,internal::rij_inter_xz[lc][j],my,internal::rij_inter_yz[lc][j],mz,internal::rij_inter_zz[lc][j],vmpi::my_rank);
            		}
            		else{
                     ...
                     //fprintf(stderr,"\tmx(%d) %f my(%d) %f mz(%d) %f\n\tfield_x = %f\tfield_y = %f\tfield_z = %f on rank = %d\n",j,mx,j,my,j,mz,dipole::cells_field_array_x[i],dipole::cells_field_array_y[i],dipole::cells_field_array_z[i],vmpi::my_rank);
                     fprintf(stderr,"\t%f*%f %f*%f %f*%f\n\t%f*%f %f*%f %f*%f\n\t%f*%f %f*%f %f*%f\n\t on rank = %d\n",mx,internal::rij_intra_xx[lc][j],my,internal::rij_intra_xy[lc][j],mz,internal::rij_intra_xz[lc][j],mx,internal::rij_intra_xy[lc][j],my,internal::rij_intra_yy[lc][j],mz,internal::rij_intra_yz[lc][j],mx,internal::rij_intra_xz[lc][j],my,internal::rij_intra_yz[lc][j],mz,internal::rij_intra_zz[lc][j],vmpi::my_rank);
            		}
          		}
               fprintf(stderr," >> sum of dipolar field on cell:\n >> i=%d magx=%f magy=%f magz=%f fieldx=%f fieldy=%f fieldz=%f rank=%d\n",i,cells::mag_array_x[i],cells::mag_array_y[i],cells::mag_array_z[i],dipole::cells_field_array_x[i],dipole::cells_field_array_y[i],dipole::cells_field_array_z[i],vmpi::my_rank);
         	}
