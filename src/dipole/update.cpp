	...
	void dipole::internal::update_field(){
      ...
      if(!dipole::activated) return;
    	...
    	for(int lc=0;lc<dipole::internal::cells_num_local_cells;lc++){
         ...
         //// From B-field calculate mu_0*Hdemag field for each cell from: mu_0*H_demag = B - (mu0*Mcell/Vuc)
         //// where 4pix10^23 comes from mu_0/1e-30 to convert from Ang to meters
         //dipole::cells_mu0Hd_field_array_x[i] = dipole::cells_field_array_x[i] - (4.0e23*M_PI * cells::mag_array_x[i]) / dipole::internal::cells_volume_array[i];
         //dipole::cells_mu0Hd_field_array_y[i] = dipole::cells_field_array_y[i] - (4.0e23*M_PI * cells::mag_array_y[i]) / dipole::internal::cells_volume_array[i];
         //dipole::cells_mu0Hd_field_array_z[i] = dipole::cells_field_array_z[i] - (4.0e23*M_PI * cells::mag_array_z[i]) / dipole::internal::cells_volume_array[i];
     		}
    	}
