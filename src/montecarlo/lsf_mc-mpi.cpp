...
// checkerboard LSF-MC algorithm
//------------------------------------------------------------------------------
void lsf_mc_parallel_init(std::vector<double> &x, // atomic coordinates
                      std::vector<double> &y,
                      std::vector<double> &z,
   ...
   if(num_atoms_in_octants != catoms){
      std::cerr << "Programmer error: missing atoms in core octants in parallel monte carlo - LSF initialisation" << std::endl;
      err::vexit();
   }
   // boundary atoms
   ...
   if(num_atoms_in_octants != batoms){
      std::cerr << "Programmer error: missing atoms in boundary octants in parallel monte carlo - LSF initialisation" << std::endl;
      err::vexit();
   }

   // set flag to indicate that parallel LSF-MC has been initialised
   lsf_mc_parallel_initialized = true;

}

/// Spin move
/// Move spin within cone near old position
void mc_transverse(const std::vector<double>& old_spin, std::vector<double>& new_spin, const double angle){
...
//------------------------------------------------------------------------------
// Integrates an LSF-Monte Carlo step in parallel
//------------------------------------------------------------------------------
void lsf_mc_step_parallel(std::vector<double> &x_spin_array,
   ...
   for(int octant = 0; octant < 8; octant++) {
      ...
      for(int i=0; i<nmoves; i++){
         ...
         // Make LSF-Monte Carlo move
         mc_transverse(internal::Sold, internal::Snew, montecarlo::internal::adaptive_sigma);
